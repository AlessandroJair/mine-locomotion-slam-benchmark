...
class WaypointRecorder:
    ...
    Uso:
    1. Lanza este nodo
    2. Conduce el robot manualmente a cada posición deseada
    ...
    def __init__(self):
        ...
        rospy.loginfo("="*60)
        rospy.loginfo("Waypoint Recorder iniciado")
        rospy.loginfo("="*60)
        rospy.loginfo("Servicios disponibles:")
        rospy.loginfo("  - /record_waypoint : Graba la posición actual")
    ...
    def odom_callback(self, msg):
        """Callback para obtener la posición actual"""
        self.current_pose = msg.pose.pose
        
    def get_current_position(self):
        """Obtiene posición y orientación actuales"""
    ...
    def record_waypoint(self, req):
        ...
        if x is None:
            rospy.logwarn("No hay datos de odometría disponibles")
        ...
        rospy.loginfo(f"  Posición: ({x:.3f}, {y:.3f})")
        rospy.loginfo(f"  Orientación: {math.degrees(yaw):.2f}°")
