...
class WaypointNavigatorYAML:
    ...
    def load_config(self, config_file):
        ...
        try:
            ...
            # --- approach shaping -------------------------------------------
            # Start slowing down this far out so the robot arrives at a speed
            # it can actually stop from, instead of overshooting the tolerance
            # and having to come back.
            self.approach_distance = nav_params.get('approach_distance', 2.0)
            self.approach_speed = nav_params.get('approach_speed', 0.2)

            # --- platform kinematics ----------------------------------------
            # The Rocker-Bogie is steered: it has a minimum turn radius and
            # cannot rotate about its own centre without scrubbing all six
            # wheels sideways. Commanding a pure rotation on such a platform
            # produces a slow, juddering skid that inflates exactly the
            # vibration and slip metrics this study reports. So the follower
            # must never ask for an angular velocity the steering cannot
            # deliver at the current speed:
            #
            #     |w| <= v / R_min
            #
            # and when the heading error is too large to fix on an arc, it
            # drives forward along the tightest arc available rather than
            # stopping to spin.
            self.min_turn_radius = nav_params.get('min_turn_radius', 0.0)
            self.can_pivot_in_place = nav_params.get('can_pivot_in_place', True)
    ...
    def navigate_to_waypoint(self, waypoint):
        ...
        while not rospy.is_shutdown():
            ...
            cmd = self.compute_command(distance, angle_diff)
    ...
    def compute_command(self, distance, angle_diff):
        """Twist toward the goal, respecting the platform's turning limits.

        Three things shape the command:

        speed profile   the robot cruises at max_linear_speed until it is
                        within approach_distance of the goal, then ramps down
                        to approach_speed.  Arriving slowly is what makes a
                        0.8 m tolerance achievable without circling.

        heading gain    the linear speed is reduced when the heading error is
                        large, so the robot does not drive fast in the wrong
                        direction.  Unlike the previous version there is no
                        floor on this: a hard-coded minimum speed of 0.3 m/s
                        meant the robot kept driving forward while it was still
                        pointing up to 90 deg off course.

        turn limit      for a steered platform the yaw rate is capped at
                        v / R_min.  Asking for more than that is asking for a
                        turn the steering geometry cannot produce; the wheels
                        simply scrub, which is both slow and a false signal in
                        the vibration and slip metrics.
        """
        cmd = Twist()
        ...
        if distance > self.approach_distance:
            ...
            # Linear ramp from approach_speed at the tolerance boundary to
            # max_linear_speed at approach_distance.
            span = max(self.approach_distance - self.goal_tolerance, 1e-3)
            frac = (distance - self.goal_tolerance) / span
            frac = max(0.0, min(1.0, frac))
            v = self.approach_speed + frac * (self.max_linear_speed -
                                              self.approach_speed)

        v *= max(0.0, 1.0 - abs(angle_diff) / math.pi)
        v = min(v, self.linear_kp * distance + self.approach_speed)

        # --- yaw ---
        w = self.angular_kp * angle_diff
        w = max(-self.max_angular_speed, min(self.max_angular_speed, w))

        if not self.can_pivot_in_place and self.min_turn_radius > 0.0:
            if abs(angle_diff) > math.radians(100.0):
                # Too far off course to fix on an arc from a standstill.  Back
                # up along the tightest available arc rather than spinning:
                # reversing swings the nose the other way and costs less
                # scrubbing than a six-wheel skid pivot.
                v = -self.approach_speed
                w = math.copysign(abs(v) / self.min_turn_radius, -angle_diff)
            else:
                # Keep enough speed that the demanded yaw rate is feasible.
                v = max(v, self.approach_speed)
                w_max = abs(v) / self.min_turn_radius
    ...
    def adjust_orientation(self, goal_yaw):
        ...
        if not self.can_pivot_in_place:
            # A steered platform cannot correct its heading in place.  Forcing
            # it to try produces a long skid that contaminates the metrics and
            # often never converges, so the final heading is simply left as the
            # approach heading.  The waypoints are route markers, not poses to
            # be held, and the next segment corrects the heading anyway.
            rospy.loginfo('Steered platform: skipping in-place heading '
                          'correction (target yaw %.1f deg).',
